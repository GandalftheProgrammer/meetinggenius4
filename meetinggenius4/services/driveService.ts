...
const setAccessToken = (token: string | null) => {
  accessToken = token;
  tokenExpiresAt = token ? Date.now() + 55 * 60 * 1000 : 0; // 55 min (5 min before Google's 60 min expiry)
};
      ...
      if (response.code && uid) {
        try {
          ...
          if (!res.ok || data.error) {
            console.error("Drive exchange failed:", data.error);
            alert(`Drive connection failed: ${data.error || res.status}. Please try again.`);
...
export const ensureValidToken = async (uid?: string): Promise<string | null> => {
  ...
  if (accessToken) setAccessToken(null); // clear stale token before refreshing
  if (!uid) return null;
...
const uploadFile = async (name: string, content: string | Blob, type: string, sub: string, toDoc: boolean, uid?: string): Promise<any> => {
  ...
  let r: Response;
  try {
    r = await fetch('https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name', {
      method: 'POST',
      headers: {
