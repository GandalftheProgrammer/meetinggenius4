...
export default async (req: Request) => {
  ...
  try {
    ...
    console.error(`[Background Error] Invalid request payload: ${err.message}`);
    return;
  }
...
async function uploadToGoogle(buffer: Buffer, displayName: string, mimeType: string, apiKey: string): Promise<string> {
    ...
    if (!initResp.ok) {
        ...
        throw new Error(`Google Upload Handshake failed (${initResp.status}): ${errBody}`);
    }
    ...
    if (!finalResp.ok) {
        ...
        throw new Error(`Google Upload Finalize failed (${finalResp.status}): ${errBody}`);
    }
